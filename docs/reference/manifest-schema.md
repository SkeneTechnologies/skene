...
By default, the manifest is written to `./skene-context/growth-manifest.json` (legacy `./skene/` continues to work when already present). You can change the output path with the `--output` flag:
