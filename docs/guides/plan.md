...
The command looks for `growth-manifest.json` and `growth-template.json` in `./skene-context/` first, then the legacy `./skene/` directory, then falls back to the current directory. Neither file is required — the command runs with whatever context it finds.
...
| `--context PATH` | `-c` | Directory containing manifest and template. Auto-detected from `./skene-context/` (or legacy `./skene/`) if not specified. |
...
The middle sections (between Executive Summary and Technical Execution) are driven by a `plan-steps.md` file. Place it at `skene-context/plan-steps.md` (or legacy `skene/plan-steps.md`) or in the directory specified by `--context`.
...
The command also loads existing engine context from `<project-root>/skene-context/engine.yaml` (when present). Existing engine features are used as prior context so the plan focuses on complementary work instead of duplicating what already exists.
