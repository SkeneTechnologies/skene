"""Pydantic models for the ``Journey`` artifact — the contract for analysis output.

Reproduced from the reference journey schema so that anything the daemon emits is
validated end-to-end before it is stored. The real analyzer (dropped in later behind
the ``Analyzer`` protocol) must produce instances of these models; the stub does too.
"""
